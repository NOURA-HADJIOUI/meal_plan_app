...
class _RecipeDetailScreenState extends State<RecipeDetailScreen> with SingleTickerProviderStateMixin {
  ...
  // Liste des reviews incluant celle de l'utilisateur
  ...
  void initState() {
    ...
    // Initialiser la liste des reviews avec les reviews existantes
    _allReviews = List.from(widget.recipe.reviews);
  }
  ...
  void _submitRating() {
    if (_userSelectedStars > 0) {
      // Créer la review de l'utilisateur
      final newUserReview = Review(
        userName: 'You', // Ou utiliser le nom de l'utilisateur connecté
        userImage: 'assets/images/user_avatar.png', // Avatar par défaut ou de l'utilisateur
        rating: _userSelectedStars.toDouble(),
      ...
      setState(() {
        _hasUserRated = true;
        _userReview = newUserReview;
        
        // Si l'utilisateur avait déjà noté, remplacer son ancienne review
        _allReviews.removeWhere((review) => review.userName == 'You');
        
        // Ajouter la nouvelle review en première position
        _allReviews.insert(0, newUserReview);
      });
  ...
  Widget _buildReviewsSection() {
    ...
    // Calculer le rating moyen incluant la review de l'utilisateur
    double totalRating = 0;
    int totalReviews = _allReviews.length;
      ...
      children: [
        ...
        Row(
          ...
          children: List.generate(
            ...
            (index) => Icon(
              Icons.star,
              color: index < totalRating.floor() ? Colors.amber : Colors.amber.withOpacity(0.3),
              size: 30.0,
            ),
          ),
        ),
        ...
        const SizedBox(height: 16.0),
        if (_allReviews.isNotEmpty) _buildReviewsList(),
      ],
    );
  }

  Widget _buildRatingBars() {
    // Calcul dynamique des barres de notation basé sur toutes les reviews
    ...
    int totalReviews = _allReviews.isNotEmpty ? _allReviews.length : 100;
  ...
  Widget _buildReviewsList() {
    return ListView.builder(
      ...
      itemBuilder: (context, index) {
        ...
        final isUserReview = review.userName == 'You';
        
        return Padding(
          padding: const EdgeInsets.only(bottom: 16.0),
          child: Container(
            decoration: isUserReview ? BoxDecoration(
              color: Colors.blue.shade50,
              borderRadius: BorderRadius.circular(8.0),
              border: Border.all(color: Colors.blue.shade200),
            ) : null,
            padding: isUserReview ? const EdgeInsets.all(12.0) : EdgeInsets.zero,
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                Row(
                  children: [
                          ...
                          Row(
                            children: [
                              Text(
                                ...
                                style: TextStyle(
                                  fontWeight: FontWeight.bold,
                                  color: isUserReview ? Colors.blue.shade700 : Colors.black,
                                ),
                              ),
                              if (isUserReview) ...[
                                const SizedBox(width: 8.0),
                    ...
                    if (isUserReview)
                      IconButton(
                        ...
                        onPressed: () => _editUserReview(review),
                      ),
                  ],
                ),
                const SizedBox(height: 8.0),
                ...
                Text(
                  review.comment,
                  style: TextStyle(
                    color: isUserReview ? Colors.blue.shade800 : Colors.black87,
                  ),
                ),
                const SizedBox(height: 8.0),
                Divider(),
  ...
  void _editUserReview(Review userReview) {
    setState(() {
      _userSelectedStars = userReview.rating.round();
      _commentController.text = userReview.comment == 'No comment provided' ? '' : userReview.comment;
    });
    
    // Scroll vers la section de rating
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        content: Text('Scroll down to edit your review'),
  ...
  Widget _buildUserAvatar(String imageUrl) {
    if (imageUrl.startsWith('http') || imageUrl.startsWith('https')) {
      ...
      return CircleAvatar(
        ...
        child: imageUrl.isEmpty || imageUrl == 'assets/images/user_avatar.png' 
